...
This project is a client-side PWA plus an optional Node.js adaptation backend. Workout, diet, and schedule recommendations can be regenerated by LightweightLLM whenever metrics change. Core UI state is still stored locally in the browser.
...
- Seven-day workout split with bodyweight, dumbbell, and CrossFit filtering
- First-launch onboarding wizard for profile, goals, equipment, and diet
...
- LLM-backed plan adaptation when metrics change (`POST /api/adapt`)
...
- Equipment filter: All, Bodyweight Only, or CrossFit
- Intensity filter: Standard or Marathon Burn
- CrossFit days: MetCon (Mon), conditioning (Wed), strength (Fri)
- First-launch onboarding stored under `rebyld_user_profile`
...
```text
rebyld/
├── index.html                 Main app shell, onboarding dialog, workout/nutrition UI
├── style.css                  Visual system, layouts, cards, modal, onboarding, LLM toast
├── app.js                     Storage, filters, tracking, onboarding, LLM adaptation client
├── sw.js                      Service worker for offline shell caching
├── manifest.webmanifest       PWA install metadata
├── .htmlhintrc                HTMLHint rules used in CI
├── .env.example               Backend env template (LLM URL, model, CORS origin)
├── icons/                     App icons (192 / 512)
├── server/
│   ├── package.json           Express backend dependencies
│   ├── server.js              HTTP server, CORS, /health, /api/adapt
│   ├── llm.js                 LightweightLLM chat-completions client
│   └── routes/adapt.js        Metric-change adaptation endpoint
└── .github/
    ├── workflows/ci-cd.yml    Test, dependency-review, PR summary, GitHub Pages deploy
    └── scripts/run-checks.sh  HTMLHint, syntax, manifest, server dry-run, Lighthouse
```
...
Optional adaptive-plan backend (LightweightLLM must already be running):
...
Set `window.__REBYLD_BACKEND_URL__` in `index.html` to the deployed backend URL (GitHub Pages is static-only).
